        ...
        geographyConfig: {
            ...
            popupTemplate: function(geo, data) {
                ...
                if (absPerc == "absolute values") { 
                    ...
                    return ["<div class=hoverinfo>",
                        "<strong>", geo.properties.name, "</strong>",
                        "<br>Percentage of refugees: <strong>", Math.round(dataset[geo.id].amount * 1000) / 1000, "</strong>",
                        "</div>"].join('');
                }
            }
        },
...
function makeTwoSidedBarchart() { 
    ...
    svgTwoSided = d3.select("#container4").append("svg")
        .attr("width", widthT2 + marginT.left + marginT.right)
...
function addTitlesTwoSided() {
    ...
    svgTwoSided.append("g")
        ...
        .append("text")
            .attr("class", "axisTitle")
            .attr("x", widthT2/2 - labelArea + labelArea/2)
            .attr("y", - heightT + 25)
            .style("text-anchor", "middle")
            .text("Age");
