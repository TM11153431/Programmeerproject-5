...
function makeTwoSidedBarchart() { 
    ...
    // initialize svg
    var svgTwoSided = d3.select("#container4").append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .attr("id", "twoSided")
        .append("g")
    ...
    // set age groups
    var dataTwoSidedLeft = male
    var dataTwoSidedRight = female

    // set domain
    var xL = d3.scale.linear()
        .domain([0, d3.max(dataTwoSidedLeft)])
        .range([0, width2]);

    var yT = d3.scale.ordinal()
        .domain(ageGroups)
    ...
    var xR = d3.scale.linear()
        .domain([0, d3.max(dataTwoSidedRight)])
        .range([0, width2]);

    var yPosByIndex = function(d, index) { return yD(index); }

    svgTwoSided.selectAll("rect.left")
        .data(dataTwoSidedLeft)
        .enter().append("rect")
        .attr("x", function(pos) { return width2 - xL(pos); })
        .attr("y", yPosByIndex)
        .attr("class", "left")
        .attr("width", xL)
    ...
    svgTwoSided.selectAll("text.leftscore")
        .data(dataTwoSidedLeft)
        .enter().append("text")
        .attr("x", function(d) { return width2 - xL(d); })
    ...
    svgTwoSided.selectAll("rect.right")
        ...
        .enter().append("rect")
        .attr("x", width2 + labelArea)
        .attr("y", yPosByIndex)
        .attr("class", "right")
        .attr("width", xR)
        .attr("height", yT.rangeBand());

    svgTwoSided.selectAll("text.score")
        .data(dataTwoSidedRight)
        .enter().append("text")
        .attr("x", function(d) { return xR(d) + width2 + labelArea; })
        .attr("y", function(d, z) { return yD(z) + yD.rangeBand()/2; })
        .attr("dx", -5)
        .attr("dy", ".36em")
        .attr("text-anchor", "end")
    ...
    svgTwoSided.append("g")
        ...
        .append("text")
            ...
            .attr("y", - height - margin.top / 2)
            .style("text-anchor", "middle")
            .text("Gender and age of refugees in " + countryStarName);
};